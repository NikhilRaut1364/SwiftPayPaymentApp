...
<html>
<head>
<title>BroadBand Bill Payment</title>
<style>
*{
margin:0px;
font-family:candara;
...
<body>
<form method="POST" action="Broadband">
...
int amount=0;
String status="",baccno="",teleno="",pro="";

try{
	Class.forName("net.ucanaccess.jdbc.UcanaccessDriver");
	con=DriverManager.getConnection(url);
	Statement stmt = con.createStatement();
	ResultSet rs=stmt.executeQuery("select * from broadband where accno='"+broadaccno+"' and tno='"+tno+"'");
	while(rs.next())
	{
		amount=rs.getInt("amount");
		status=rs.getString("status");
		baccno=rs.getString("accno");
		teleno=rs.getString("tno");
		pro=rs.getString("provider");
	}
	if(!broadaccno.equals(baccno) && !tno.equals(teleno) && !pro.equals(provider))
	{%>
		<script type="text/javascript">
		alert("Wrong Input");
...
<div class="parent">
	<div class="child">
		<h1 style="margin-bottom:15px;font-size:30px;color:#666666;">BroadBand Bill Payment</h1>
