...
public class Train extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet r=stmt.executeQuery("select * from train where source='"+from+"' and destination='"+to+"' and tdate='"+fdate+"' and class='"+fclass+"'");
			while(r.next())
			{
				seat=r.getInt("seat");
			...
			if(upi.equals(uniquecode))
			{
				...
				msg+="<table height=\"200\"><tr><td colspan=\"5\"><center><b style=\"font-size:20px;\">Train Ticket</b></center></td></tr>";
				...
				if(rad.equals("debit"))
				{
					...
					if(dcardno.equals(dcno) && dcvv.equals(dcv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Train Ticket','"+dat+"','Debit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update train SET seat="+totseat+" where source='"+from+"' and destination='"+to+"' and class='"+fclass+"' and tdate='"+fdate+"'");
					...
					{
						//invalid card
						request.getRequestDispatcher("train.jsp").include(request, response);
				...
				else if(rad.equals("credit"))
				{
					...
					if(ccardno.equals(ccno) && ccvv.equals(ccv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Train Ticket','"+dat+"','Credit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update train SET seat="+totseat+" where source='"+from+"' and destination='"+to+"' and class='"+fclass+"' and tdate='"+fdate+"'");
							Mailer.send(uemail, subject, msg);
							response.sendRedirect("transactionSuccessfull.html");
						}
				...
				else if(rad.equals("wallet"))
				{
					...
					else
					{
						...
						stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Train Ticket','"+dat+"','Wallet',"+amount+","+uaccno+")");
						stmt.executeUpdate("update flight SET seat="+totseat+" where source='"+from+"' and destination='"+to+"' and class='"+fclass+"' and tdate='"+fdate+"'");
						Mailer.send(uemail, subject, msg);
						response.sendRedirect("transactionSuccessfull.html");
					}
				}
