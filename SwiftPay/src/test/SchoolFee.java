...
public class SchoolFee extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		msg+="<table height=\"300\"><tr><td><center><b style=\"font-size:20px;\">School Fee</b></center></td></tr>";
		msg+="<tr><td>Admission Number: "+ano+"</td></tr>";
		msg+="<tr><td>School Name: "+sname+"</td></tr>";
		msg+="<tr><td>Amount: "+amount+"</td></tr>";
		msg+="</table></body></html>";
		
		String subject="School Fee Challan";
			...
			if(upi.equals(uniquecode))
			{
				...
				if(rad.equals("debit"))
				{
					...
					if(dcardno.equals(dcno) && dcvv.equals(dcv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','School Fees','"+dat+"','Debit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update schoolfee SET status='paid' where ano='"+ano+"' and sname='"+sname+"'");
					...
					{
						//invalid card
						request.getRequestDispatcher("schoolfees.jsp").include(request, response);
				...
				else if(rad.equals("credit"))
				{
					...
					if(ccardno.equals(ccno) && ccvv.equals(ccv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','School Fees','"+dat+"','Credit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update schoolfee SET status='paid' where ano='"+ano+"' and sname='"+sname+"'");
							Mailer.send(to, subject, msg);
							response.sendRedirect("transactionSuccessfull.html");
						}
				...
				else if(rad.equals("wallet"))
				{
					...
					else
					{
						...
						stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','School Fees','"+dat+"','Wallet',"+amount+","+uaccno+")");
						stmt.executeUpdate("update schoolfee SET status='paid' where ano='"+ano+"' and sname='"+sname+"'");
						Mailer.send(to, subject, msg);
						response.sendRedirect("transactionSuccessfull.html");
					}
