...
/**
 * Servlet implementation class Broadband
 */
public class Broadband extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String broadaccno=request.getParameter("accno");
			...
			if(upi.equals(uniquecode))
			{
				if(rad.equals("debit"))
				{
					...
					if(dcardno.equals(dcno) && dcvv.equals(dcv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Broadband','"+dat+"','Debit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update broadband SET status='paid' where accno='"+broadaccno+"' and tno='"+tno+"' and provider='"+provider+"'");
					...
					{
						//invalid card
						request.getRequestDispatcher("broadband1.jsp").include(request, response);
				...
				else if(rad.equals("credit"))
				{
					...
					if(ccardno.equals(ccno) && ccvv.equals(ccv))
					{
						...
						else
						{
							...
							stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Broadband','"+dat+"','Credit Card',"+amount+","+uaccno+")");
							stmt.executeUpdate("update broadband SET status='paid' where accno='"+broadaccno+"' and tno='"+tno+"' and provider='"+provider+"'");
							response.sendRedirect("transactionSuccessfull.html");
						}
					}
				...
				else if(rad.equals("wallet"))
				{
					...
					else
					{
						...
						stmt.executeUpdate("insert into transaction(uname,tname,tdate,pmethod,amount,accno)values('"+name+"','Broadband','"+dat+"','Wallet',"+amount+","+uaccno+")");
						stmt.executeUpdate("update broadband SET status='paid' where accno='"+broadaccno+"' and tno='"+tno+"' and provider='"+provider+"'");
						response.sendRedirect("transactionSuccessfull.html");
					}
				}
